...
void main()
{
    ...
    /*
    vec3 camera_pos = vec3(inverse(view_trans)[3]);
    vec3 inf_offset = vec3(inverse(view_trans) * vec4(0., 0., pow(2, 20), 0.));
    ...
    */
    ...
    vec3 camera_pos = vec3(view_trans[3]);
    camera_pos.z = pow(2, 20);
    camera_pos = vec3(inverse(view_trans) * vec4(camera_pos, 1.0));
